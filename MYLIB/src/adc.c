...
//----------------------------------------------------------------------------------------------------------------------------
	...
	while (DMA_GetCmdStatus(DMA2_Stream0) != DISABLE){}//µÈ´ıDMA¿ÉÅäÖÃ 
	
  /* ÅäÖÃ DMA Stream */
  dma_init_structure.DMA_Channel = 0;  //Í¨µÀÑ¡Ôñ
  dma_init_structure.DMA_PeripheralBaseAddr = (uint32_t) &(ADC1->DR);//DMAÍâÉèµØÖ·
  dma_init_structure.DMA_Memory0BaseAddr = (uint32_t)&adc_dma_tab[0];;//DMA ´æ´¢Æ÷0µØÖ·
  dma_init_structure.DMA_DIR = DMA_DIR_MemoryToPeripheral;//´æ´¢Æ÷µ½ÍâÉèÄ£Ê½
  dma_init_structure.DMA_BufferSize = 6;//Êı¾İ´«ÊäÁ¿ 
  dma_init_structure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//ÍâÉè·ÇÔöÁ¿Ä£Ê½
  dma_init_structure.DMA_MemoryInc = DMA_MemoryInc_Enable;//´æ´¢Æ÷ÔöÁ¿Ä£Ê½
  dma_init_structure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;//ÍâÉèÊı¾İ³¤¶È:8Î»
  dma_init_structure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;//´æ´¢Æ÷Êı¾İ³¤¶È:8Î»
  dma_init_structure.DMA_Mode = DMA_Mode_Normal;// Ê¹ÓÃÆÕÍ¨Ä£Ê½ 
  dma_init_structure.DMA_Priority = DMA_Priority_Medium;//ÖĞµÈÓÅÏÈ¼¶
  dma_init_structure.DMA_FIFOMode = DMA_FIFOMode_Disable;         
  dma_init_structure.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;
  dma_init_structure.DMA_MemoryBurst = DMA_MemoryBurst_Single;//´æ´¢Æ÷Í»·¢µ¥´Î´«Êä
  dma_init_structure.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;//ÍâÉèÍ»·¢µ¥´Î´«Êä
  DMA_Init(DMA2_Stream0, &dma_init_structure);//³õÊ¼»¯DMA Stream
		
	DMA_Cmd(DMA2_Stream0, DISABLE);                      //¹Ø±ÕDMA´«Êä 
	
	while (DMA_GetCmdStatus(DMA2_Stream0) != DISABLE){}	//È·±£DMA¿ÉÒÔ±»ÉèÖÃ  
		
	DMA_SetCurrDataCounter(DMA2_Stream0,6);          //Êı¾İ´«ÊäÁ¿  
...
u16 Get_Adc(u8 ch)   
{
	...
	ADC_SoftwareStartConv(ADC1);		//Ê¹ÄÜÖ¸¶¨µÄADC1µÄÈí¼ş×ª»»Æô¶¯¹¦Ä
